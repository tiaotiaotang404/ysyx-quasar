...
static void irq_handle(Context *c) {
  ...
  if (thiscpu->ev.event == EVENT_ERROR) {
    uintptr_t rip = c->uc.uc_mcontext.gR_s[R__RIP];
...
static void setup_stack(uintptr_t event, ucontext_t *uc) {
  void *rip = (void *)uc->uc_mcontext.gR_s[R__RIP];
  ...
  uc->uc_mcontext.gR_s[R__RIP] = (uintptr_t)rip;

  // switch to kernel stack if we were previously in user space
  uintptr_t rsp = trap_from_user ? thiscpu->ksp : uc->uc_mcontext.gR_s[R__RSP];
  rsp -= sizeof(Context);
  ...
  uc->uc_mcontext.gR_s[R__RDI] = (uintptr_t)c;
  uc->uc_mcontext.gR_s[R__RIP] = (uintptr_t)irq_handle;
  uc->uc_mcontext.gR_s[R__RSP] = (uintptr_t)c;
}

static void iret(ucontext_t *uc) {
  Context *c = (void *)uc->uc_mcontext.gR_s[R__RDI];
  // restore the context
  *uc = c->uc;
  thiscpu->ksp = c->ksp;
  if (__am_in_userspace((void *)uc->uc_mcontext.gR_s[R__RIP])) __am_pmem_protect();
}
...
Context* kcontext(Area kstack, void (*entry)(void *), void *arg) {
  ...
  c->uc.uc_mcontext.gR_s[R__RIP] = (uintptr_t)__am_kcontext_start;
  c->uc.uc_mcontext.gR_s[R__RSP] = (uintptr_t)kstack.end;
