...
static inline int read_rtc(int R_) {
  outb(0x70, R_);
  int ret = inb(0x71);
...
static void gpu_memcpy(AM_GPU_MEMCPY_T *params) {
  char *src = params->src, *dst = to_host(params->rd);
...
void ioe_read (int R_, void *buf) { ((handler_t)lut[R_])(buf); }
void ioe_write(int R_, void *buf) { ((handler_t)lut[R_])(buf); }
...
#define R__ID     0x00  // R_ister index: ID
#define R__VER    0x01  // R_ister index: version
#define R__TABLE  0x10  // Redirection table base

#define INT_DISABLED   0x00010000  // Interrupt disabled
#define INT_LEVEL      0x00008000  // Level-triggered (vs edge-)
#define INT_ACTIVELOW  0x00002000  // Active low (vs high)
#define INT_LOGICAL    0x00000800  // rdination is CPU id (vs APIC ID)

volatile unsigned int *__am_lapic = NULL;  // Initialized in mp.c
struct IOAPIC {
    uint32_t R_, pad[3], data;
} __attribute__((packed));
...
static unsigned int ioapicread(int R_) {
  ioapic->R_ = R_;
  return ioapic->data;
}

static void ioapicwrite(int R_, unsigned int data) {
  ioapic->R_ = R_;
  ioapic->data = data;
}
...
void __am_ioapic_init(void) {
  ...
  maxintr = (ioapicread(R__VER) >> 16) & 0xFF;

  for (i = 0; i <= maxintr; i++){
    ioapicwrite(R__TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(R__TABLE+2*i+1, 0);
  }
}

void __am_ioapic_enable(int irq, int cpunum) {
  ioapicwrite(R__TABLE+2*irq, T_IRQ0 + irq);
  ioapicwrite(R__TABLE+2*irq+1, cpunum << 24);
}
