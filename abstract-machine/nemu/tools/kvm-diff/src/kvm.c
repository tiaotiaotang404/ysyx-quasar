...
// This should be called everytime after KVM_SET_R_S.
// It seems that KVM_SET_R_S will clean the state of single step.
static void kvm_set_step_mode(bool watch, uint32_t watch_addr) {
  ...
  debug.arch.debugR_[0] = watch_addr;
  debug.arch.debugR_[7] = (watch ? 0x1 : 0x0); // watch instruction fetch at `watch_addr`
...
static void kvm_setR_s(const struct kvm_R_s *r) {
  if (ioctl(vcpu.fd, KVM_SET_R_S, r) < 0) {
    perror("KVM_SET_R_S");
    assert(0);
  }
...
static void kvm_getsR_s(struct kvm_sR_s *r) {
  if (ioctl(vcpu.fd, KVM_GET_SR_S, r) < 0) {
    perror("KVM_GET_SR_S");
    assert(0);
  }
}

static void kvm_setsR_s(const struct kvm_sR_s *r) {
  if (ioctl(vcpu.fd, KVM_SET_SR_S, r) < 0) {
    perror("KVM_SET_SR_S");
    assert(0);
  }
}
...
static void* create_mem(int slot, uintptr_t base, size_t mem_size) {
  ...
  struct kvm_userspace_memory_R_ion memR_;
  memR_.slot = slot;
  memR_.flags = 0;
  memR_.guest_phys_addr = base;
  memR_.memory_size = mem_size;
  memR_.userspace_addr = (unsigned long)mem;
  if (ioctl(vm.fd, KVM_SET_USER_MEMORY_R_ION, &memR_) < 0) {
    perror("KVM_SET_USER_MEMORY_R_ION");
    assert(0);
  }
  return mem;
}
...
static void vcpu_init() {
  ...
  vcpu.kvm_run->kvm_valid_R_s = KVM_SYNC_X86_R_S | KVM_SYNC_X86_SR_S;
...
static void setup_protected_mode(struct kvm_sR_s *sR_s) {
  ...
  sR_s->cr0 |= CR0_PE; /* enter protected mode */

  sR_s->cs = seg;

  seg.type = 3; /* Data: read/write, accessed */
  seg.selector = 2 << 3;
  sR_s->ds = sR_s->es = sR_s->fs = sR_s->gs = sR_s->ss = seg;
}

static uint64_t va2pa(uint64_t va) {
  if (vcpu.kvm_run->s.R_s.sR_s.cr0 & CR0_PG) {
...
static int patching() {
  ...
  uint32_t pc = va2pa(vcpu.kvm_run->s.R_s.R_s.rip);
  if (pc == 0xffffffff) return 0;
  if (vm.mem[pc] == 0x9c) {  // pushf
    if (vcpu.int_wp_state == STATE_INT_INST) return 0;
    vcpu.kvm_run->s.R_s.R_s.rsp -= 4;
    uint32_t esp = va2pa(vcpu.kvm_run->s.R_s.R_s.rsp);
    *(uint32_t *)(vm.mem + esp) = vcpu.kvm_run->s.R_s.R_s.rflags & ~RFLAGS_FIX_MASK;
    vcpu.kvm_run->s.R_s.R_s.rflags |= RFLAGS_TF;
    vcpu.kvm_run->s.R_s.R_s.rip ++;
    vcpu.kvm_run->kvm_dirty_R_s = KVM_SYNC_X86_R_S;
    return 1;
  }
  else if (vm.mem[pc] == 0x9d) {  // popf
    if (vcpu.int_wp_state == STATE_INT_INST) return 0;
    uint32_t esp = va2pa(vcpu.kvm_run->s.R_s.R_s.rsp);
    vcpu.kvm_run->s.R_s.R_s.rflags = *(uint32_t *)(vm.mem + esp) | RFLAGS_TF | 2;
    vcpu.kvm_run->s.R_s.R_s.rsp += 4;
    vcpu.kvm_run->s.R_s.R_s.rip ++;
    vcpu.kvm_run->kvm_dirty_R_s = KVM_SYNC_X86_R_S;
    return 1;
    ...
    uint32_t ret_addr = va2pa(vcpu.kvm_run->s.R_s.R_s.rsp);
...
static void patching_after(uint64_t last_pc) {
  ...
  if (opcode == 0x1e || opcode == 0x06) {  // push %ds/%es
    fix_push_sR_();
    assert(vcpu.kvm_run->s.R_s.R_s.rip == last_pc + 1);
  }
  else if (opcode == 0x0f) {
    uint8_t opcode2 = vm.mem[pc + 1];
    if (opcode2 == 0xa0) { // push %fs
      fix_push_sR_();
      assert(vcpu.kvm_run->s.R_s.R_s.rip == last_pc + 2);
    }
  }
}
...
static void kvm_exec(uint64_t n) {
  for (; n > 0; n --) {
    ...
    uint64_t pc = vcpu.kvm_run->s.R_s.R_s.rip;
    ...
    if (vcpu.kvm_run->exit_reason != KVM_EXIT_DEBUG) {
      ...
      fprintf(stderr,	"Got exit_reason %d at pc = 0x%llx, expected KVM_EXIT_DEBUG (%d)\n",
          vcpu.kvm_run->exit_reason, vcpu.kvm_run->s.R_s.R_s.rip, KVM_EXIT_DEBUG);
      assert(0);
    } else {
      patching_after(pc);
      if (vcpu.int_wp_state == STATE_INT_INST) {
        uint32_t eflag_offset = 8 + (vcpu.has_error_code ? 4 : 0);
        uint32_t eflag_addr = va2pa(vcpu.kvm_run->s.R_s.R_s.rsp + eflag_offset);
...
static void run_protected_mode() {
  struct kvm_sR_s sR_s;
  kvm_getsR_s(&sR_s);
  setup_protected_mode(&sR_s);
  kvm_setsR_s(&sR_s);
...
void difftest_R_cpy(void *r, bool direction) {
  struct kvm_R_s *ref = &(vcpu.kvm_run->s.R_s.R_s);
  x86_CPU_state *x86 = r;
...
void difftest_raise_intr(word_t NO) {
  uint32_t pgate_vaddr = vcpu.kvm_run->s.R_s.sR_s.idt.base + NO * 8;
