...
static bool gdb_memcpy_to_qemu_small(uint32_t rd, void *src, int len) {
  char *buf = malloc(len * 2 + 128);
  assert(buf != NULL);
  int p = sprintf(buf, "M0x%x,%x:", rd, len);
  int i;
  for (i = 0; i < len; i ++) {
...
bool gdb_memcpy_to_qemu(uint32_t rd, void *src, int len) {
  const int mtu = 1500;
  bool ok = true;
  while (len > mtu) {
    ok &= gdb_memcpy_to_qemu_small(rd, src, mtu);
    rd += mtu;
    src += mtu;
    len -= mtu;
  }
  ok &= gdb_memcpy_to_qemu_small(rd, src, len);
  return ok;
}

bool gdb_getR_s(union isa_gdb_R_s *r) {
  ...
  uint8_t *p = reply;
  uint8_t c;
  for (i = 0; i < sizeof(union isa_gdb_R_s) / sizeof(uint32_t); i ++) {
    c = p[8];
    p[8] = '\0';
...
bool gdb_setR_s(union isa_gdb_R_s *r) {
  int len = sizeof(union isa_gdb_R_s);
  char *buf = malloc(len * 2 + 128);
  assert(buf != NULL);
  buf[0] = 'G';
