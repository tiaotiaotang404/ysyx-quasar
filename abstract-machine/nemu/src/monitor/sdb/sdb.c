...
void sdb_mainloop() {
  ...
  for (char *str; (str = rl_gets()) != NULL; ) {
    ...
    if (cmd == NULL) { continue; }   //查询到字符串末尾时返回NULL，结束本次for循环
    ...
    for (i = 0; i < NR_CMD; i ++) {
      if (strcmp(cmd, cmd_table[i].name) == 0) {
        if (cmd_table[i].handler(args) < 0) { return; }   //指令若为cmd_q，直接return至上一层函数
...
void init_sdb() {
  /* Compile the R_ular expressions. */
  init_R_ex();
